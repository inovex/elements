...
import { Components, SortDirectionChangeDetails } from '@inovex.de/elements';
...
export default {
  ...
  component: 'ino-table',
  inline: true,
  decorators: [(story) => decorateStoryWithClass(story, 'story-ino-table')],
  argTypes: {
    sortColumnId: {
...
>('ino-table', () => {
  ...
  return html`
    <ino-table class="selectable-table" @sortChange="${sortChangeHandler}">
      ${unsafeHTML(/*html*/ `
        <tr slot="header-row">
...
export function sortChangeHandler(e: CustomEvent<SortDirectionChangeDetails>) {
  const tableEl = e.target as HTMLInoTableElement;
  const { columnId, sortDirection } = e.detail;
  tableEl.sortColumnId = columnId;
  tableEl.sortDirection = sortDirection;
}
