...
Most components have built in styles and themes you can choose from. Lets add a Notification, everytime our user signs in.
...
The `type` property of the `<InoSnackbar>` has 3 types: `info`, `error`, `success`. Change the type accordingly to your wishes. The timeout property is in *ms* and indicates when the snackbar dissapears. The timeout can be disabled by setting it to a negative value.
...
The `<InoFabSet>` has 4 properties `open-dial`, `dialDirection`, `leftRightLocation` and `topBottomLocation`. In the [Playground](https://elements.inovex.de/version/v7.0.0/?path=/docs/buttons-ino-fab-set--playground) we can find the _Description_ and _Control_ for each property.
