...
3. If a new version is available, open the `package.json` located in the inovex Elements package directory and replace
...
5. Run `pnpm start` to check if the build succeeds and the Storybook opens in the browser.
