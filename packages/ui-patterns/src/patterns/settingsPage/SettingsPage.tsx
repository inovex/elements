...
function useDisableButton<Fields extends Record<string, string>, Key extends keyof Fields>(
...
const SettingsPage = () => {
  const setPersonalInfoForm = useDisableButton('ino-button#submit-personal-info', {
    firstName: '',
    lastName: '',
    companyName: '',
    industry: '',
  ...
  function setupEventListeners(container: HTMLElement) {
    const cleanUpNavDrawer = setupEventListener(container, 'ino-nav-drawer', 'openChange', 'open');
    const cleanupFirstName = setupEventListener(
      container,
      'ino-input[label="First Name"]',
      'valueChange',
      'value',
      ev => setPersonalInfoForm('firstName', (ev as CustomEvent<string>).detail),
    );
    const cleanupLastName = setupEventListener(container, 'ino-input[label="Last Name"]', 'valueChange', 'value', ev =>
      setPersonalInfoForm('lastName', (ev as CustomEvent<string>).detail),
    );
    const cleanupCompanyName = setupEventListener(container, 'ino-input[name="company"]', 'valueChange', 'value', ev =>
      setPersonalInfoForm('companyName', (ev as CustomEvent<string>).detail),
    );
    const cleanupIndustry = setupEventListener(container, 'ino-input[label="Industry"]', 'valueChange', 'value', ev =>
      setPersonalInfoForm('industry', (ev as CustomEvent<string>).detail),
    );
    const cleanupRole = setupEventListener(container, 'ino-input[label="Role"]', 'valueChange', 'value', ev =>
      setPersonalInfoForm('role', (ev as CustomEvent<string>).detail),
    );
    const cleanupEmail = setupEventListener(container, 'ino-input[label="E-mail"]', 'valueChange', 'value');
    const cleanupCurrentPassword = setupEventListener(
      container,
      'ino-input[label="Current password"]',
      'valueChange',
      'value',
      ev => setPasswordForm('currentPassword', (ev as CustomEvent<string>).detail),
    );
    const cleanupNewPassword = setupEventListener(
      ...
      'valueChange',
      'value',
      ev => setPasswordForm('newPassword', (ev as CustomEvent<string>).detail),
    );
    const cleanupConfirmPassword = setupEventListener(
      ...
      'valueChange',
      'value',
      ev => setPasswordForm('confirmPassword', (ev as CustomEvent<string>).detail),
    );
    ...
    const cleanupSecurityAnswer1 = setupEventListener(
      ...
      'valueChange',
      'value',
      ev => setSecurityQuestionForm('answer1', (ev as CustomEvent<string>).detail),
    );
    const cleanupSecurityAnswer2 = setupEventListener(
      ...
      'valueChange',
      'value',
      ev => setSecurityQuestionForm('answer2', (ev as CustomEvent<string>).detail),
    );
    const cleanupSecurityAnswer3 = setupEventListener(
      ...
      'valueChange',
      'value',
      ev => setSecurityQuestionForm('answer3', (ev as CustomEvent<string>).detail),
    );

    const cleanupTabBar = setupEventListener(container, 'ino-tab-bar', 'activeTabChange', 'active-tab', ev =>
      onTabChange((ev as CustomEvent<number>).detail),
    );
  ...
  return <PatternWrapper htmlContent={htmlContent} onMount={setupEventListeners} />;
};
