...
export async function getGitHubContributers(): Promise<ElementsContributor[]> {
  const contributorsResponse: GithubContributor[] = await fetch(GITHUB_REPO_URL + '/contributors').then(resp =>
    resp.json(),
  );
  ...
  const developers: ElementsContributor[] = contributorsResponse
    ...
    .map(c => {
      const found = CONTRIBUTORS.find(contributor => c.login === contributor.login);
...
const getDesigners = async () => {
  ...
  await Promise.all(
    CONTRIBUTORS.filter(c => c.roles.includes(Role.DESIGN)).map(async c => {
      designerGithubProfile.push(await fetch(GITHUB_USERS_API + c.login).then(res => res.json()));
    }),
  );
