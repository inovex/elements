...
export default function CopyButtonReact({ rawCode, copyIconId }: CopyButtonProps) {
  ...
  const handleIconClick = () => {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(ACCORDION_REACT_CODE);
    }
  };
  ...
  return (
    <>
      {wasCopied ? (
        <InoIcon icon="checkmark" className={styles.copyIcon} onClick={handleIconClick} />
      ) : (
        <img src="/react.svg" style={{ width: 20 }} onClick={handleIconClick} />
      )}
      <InoPopover controlled={true} visible={wasCopied} for={copyIconId} trigger="click" placement="top">
