...
<MenuSection title='Preparation'>
  ...
  We provide an integration layer package for Angular, so you can integrate the inovex Elements as an Angular library
  ...
  <StepIndicator step='2'>If you used npm or yarn to install the elements, you need to change `angular.json` manually.
    Add the following to your build assets (`projects.
    <your-project-name>.architect.build.options.assets`) and to your test assets (`projects.
      <your-project-name>.architect.test.options.assets`).
  </StepIndicator>
...
<MenuSection title='Importing Modules'>
  ...
  This allows your IDE and compiler to infer all the type information from the given interfaces which is essential for
  code completion to work properly.

  To use the inovex Elements, you need to integrate the `InoElementsModule` into one of your App's modules. To do so,
  follow these steps:
  ...
  If you have more than one module, you should integrate the `InoElementsModule` into each module that is supposed to
  use the inovex Elements.
