...
<InoNavMenuSection sectionName='Integration via script tag'>
...
For most of the websites it should be sufficient to add the tag to the `index.html` file (choose legacy for older browsers):</StepIndicator>
<CH.Code>
  ```html Modern
  <script
    type="module"
...
**Fetching the proper version:** Please note the version number (`8.0.0`) in the url above. This version might not be the latest version in future, so please update it with latest available version number on `elements.inovex.de` during your initial integration.
...
However, for testing purposes, and if you just want to play around with the inovex Elements, you can always replace the version number with `latest`:
...
<InoNavMenuSection sectionName='Use the Components'>
