...
<article>
  ...
  export default App;
  ```
  <p className="hint">Hint: For the Styling and Structure of your App we recommend following along with the provided [codesandbox](https://codesandbox.io/s/github/inovex/elements-example-react).</p>
...
| `value`          | The value of this input field.                                                                             | `string`                                                                                          | `''`       |
    ...
    <p className="hint">Hint: These are some of the properties of the `<InoInput>` component.</p>
...
| inoFocus      | Emits when the input field is focused                                          | CustomEvent\<void>   |
  ...
  <p className="hint">Hint: while in our Playground the Event is called `valueChange`, we name it `valueChange` inside of our `<InoInput>`. The reason for this is that we want to accomodate our naming similar to the react-way of naming events (e.g. onClick)</p>
...
| **icon-trailing** | For the icon to be appended  |
  ...
  <p className="hint">Hint: We can choose the position of our slot, that will render the `<InoIcon>` component. Other components like the `<InoCard>` allow a `header`, `content`, `footer` approach of positioning its slots.</p>
  ...
  <p className="hint">Hint: automatically style your logo with rounded borders by using the `rounded` property of `<InoImg>`</p>
...
<NavigationMenu sections={reactSections}/>
