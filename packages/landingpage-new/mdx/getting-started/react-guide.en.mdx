...
<StepIndicator step='1'/> First, create a new React app for our login page.
...
<StepIndicator step='2'/> Add the package `@inovex.de/elements-react` to your project using **npm** or **yarn**:
<CH.Code>
  ```bash npm
...
<StepIndicator step='3'/> Once our React app is ready, delete the boilerplate code and restructure the function until our App.js looks something like this:
...
<StepIndicator step='4'/> Following up on our login-form, we add a type and value property, setting it to a state object named _email_. Do the same for the password `<InoInput>`.
